...
- new LES Smagorinsky and WALE models have been developed for Pb_Multiphase. Any real case should still be run in the TrioCFD derived application (CMFD module), but this helps non regression testing on the TRUST side;
- a series of problems XXX_List_Concentration & XXX_List_Concentration_Turbulent have been added, and can be used to solve a list of N arbitrary concentration equations. It is notably now possible to define different diffusion coefficients for each concentration equation (which is not the case when using list_scalaire_passif problem !!!)
- similarly, a series of problems XXX_Cloned_Concentration & XXX_Cloned_Concentration_Turbulent can be used to state only a **single concentration equation** in the .data, but clone N equations. We assume here that the same boundary conditions are used. It is still possible to define different diffusion coefficients for each cloned equation.
...
- Update PETSc to 3.20.0 (dependencies like MUMPS,... not updated yet)
...
- the ICoCo resetTime() method has been implemented, allowing to reset the time origin during the computation. This functionality is still under development, and notably the way postprocessing files are handled might still evolve in the future;
